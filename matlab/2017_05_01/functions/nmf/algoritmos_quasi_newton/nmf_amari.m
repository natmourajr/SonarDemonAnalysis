...
errs = zeros(niter,1);
traco_w= zeros(niter,1); %colocado no programa
for t = 1:niter
    ...
    errs(t) = sum(V(:).^alpha .* R(:).^(1-alpha) - alpha*V(:) + ...
                  (alpha-1)*R(:)) / (alpha*(alpha-1));
              %**************Inicio da modificação programa por mim*******
              Traco_matrix=trace(W); %colocado no programa
              traco_w(t)=Traco_matrix;
    ...
    if verb >= 3
        if(t==1)
            ...
            valor=(errs(t-1)-errs(t))/(errs(1)-errs(t-1));
            disp(['nmf_amari: iter=' num2str(t) ', err=' num2str(errs(t)) ',errs=' num2str(valor)]); %colocado por mim
        end
    end
